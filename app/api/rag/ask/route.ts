...
export async function POST(req: NextRequest) {
  try {
    ...
    // 컨텍스트가 비어있으면 안내 메시지 반환 (빈 응답 방지)
    if (!contexts || contexts.length === 0) {
      const msg = [
        '선택한 노트북에서 관련 근거를 찾지 못했습니다.\n',
        '- 먼저 파일(txt, md, pdf)을 업로드해 주세요',
        '- 또는 "/api/rag/ingest-gardner"를 호출해 기본 지식을 인덱싱할 수 있어요',
        '\n업로드 후 다시 질문하시면 근거와 함께 답변해 드립니다.'
      ].join('\n');
    ...
    let answer = '';
    try {
      const aiRes = await fetch(LMSTUDIO_URL.replace('localhost', '127.0.0.1'), {
        method: 'POST',
      ...
      if (!answer || answer.trim().length < 5) {
        // 빈 응답이면 강제 폴백
        throw new Error('Empty content from LM Studio');
      }
    } catch {
      // LM 실패 시, 컨텍스트 요약형 응답
